...
impl SchemaLineage {
    pub fn build(schema: &ResolvedSchema) -> Self {
        let latest = schema.versions.last()
            .expect("schema must have at least one version");

        let versions = schema.versions.iter().map(|ver| {
            build_version_lineage(ver, latest)
        }).collect();
    ...
    pub fn build_from(versions: &[ResolvedVersion]) -> Self {
        ...
        let version_lineages = versions.iter()
...
fn build_version_lineage(
    ...
    let latest_by_id: std::collections::HashMap<FieldId, &FieldIR> = latest
        .fields.iter()
        .map(|f| (f.id, f))
        .collect();

    let ver_ids: std::collections::HashSet<FieldId> = ver
        .fields.iter()
        .map(|f| f.id)
        .collect();

    let fields = ver.fields.iter().map(|src| {
        let mapping = match latest_by_id.get(&src.id) {
            ...
            Some(dst) => {
                if src.ty == dst.ty {
                    ...
                    FieldMapping::Cast {
                        target_name: dst.name.clone(),
                        from: src.ty.type_id().clone(),
                        to: dst.ty.type_id().clone(),
                    }
                }
            }
        };
    ...
    let missing = latest.fields.iter()
