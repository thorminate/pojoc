use criterion::{criterion_group, criterion_main, Criterion};
use std::time::Duration;
use std::hint::black_box;

use pojoc_tests::{
    pojoc_player::{Player, Vector3, encode, decode as pojoc_decode, pojvec},
    proto_player,
    player_capnp,
    fb_player,
};

use prost::Message;
use capnp::message::ReaderOptions;

fn make_proto() -> proto_player::Player {
    proto_player::Player {
        player_id: 1.0,
        level: 12.5,
        inventory: vec![
            "sword".into(),
            "shield".into(),
            "healing_potion".into(),
        ],
        position: Some(proto_player::Vector3 {
            x: 10.0,
            y: 42.5,
            z: -3.0,
            w: 1.0,
        }),
        tags: vec![
            "starter_zone".into(),
            "pvp_enabled".into(),
            "vip".into(),
        ],
        is_nauseous: false,
    }
}

fn decode_pojoc(c: &mut Criterion) {
    let mut buf = Vec::new();
    encode(&mut buf, &Player {
        player_id: 1.0,
        level: 12.5,
        inventory: pojvec!["sword", "shield", "healing_potion"],
        position: Vector3 {
            x: 10.0, y: 42.5, z: -3.0, w: 1.0
        },
        tags: pojvec!["starter_zone", "pvp_enabled", "vip"],
        is_nauseous: false,
        is_dead: true,
    });

    c.bench_function("pojoc decode", |b| {
        b.iter(|| {
            let decoded = pojoc_decode(black_box(&buf)).unwrap();
            black_box(decoded);
        })
    });
}

fn decode_proto(c: &mut Criterion) {
    let player = make_proto();
    let buf = player.encode_to_vec();

    c.bench_function("protobuf decode", |b| {
        b.iter(|| {
            let decoded =
                proto_player::Player::decode(black_box(buf.as_slice())).unwrap();
            black_box(decoded);
        })
    });
}

fn decode_capnp(c: &mut Criterion) {
    let mut message = capnp::message::Builder::new_default();
    {
        let mut p = message.init_root::<player_capnp::player::Builder>();
        p.set_player_id(1.0);
        p.set_level(12.5);
        p.set_is_nauseous(false);
    }

    let mut buf = Vec::new();
    capnp::serialize::write_message(&mut buf, &message).unwrap();

    c.bench_function("capnp decode", |b| {
        b.iter(|| {
            let reader = capnp::serialize::read_message(
                black_box(buf.as_slice()),
                ReaderOptions::new(),
            ).unwrap();

            let player =
                reader.get_root::<player_capnp::player::Reader>().unwrap();

            black_box(player);
        })
    });
}

fn decode_fb(c: &mut Criterion) {
    let mut builder = flatbuffers::FlatBufferBuilder::with_capacity(256);

    let inventory = vec![
        builder.create_string("sword"),
        builder.create_string("shield"),
        builder.create_string("healing_potion"),
    ];

    let tags = vec![
        builder.create_string("starter_zone"),
        builder.create_string("pvp_enabled"),
        builder.create_string("vip"),
    ];

    let inv_vec = builder.create_vector(&inventory);
    let tags_vec = builder.create_vector(&tags);

    let position = fb_player::Vector3::new(10.0, 42.5, -3.0, 1.0);

    let player = fb_player::Player::create(&mut builder, &fb_player::PlayerArgs {
        player_id: 1.0,
        level: 12.5,
        inventory: Some(inv_vec),
        position: Some(&position),
        tags: Some(tags_vec),
        is_nauseous: false,
    });

    builder.finish(player, None);
    let buf = builder.finished_data().to_vec();

    c.bench_function("flatbuffers decode", |b| {
        b.iter(|| {
            let p = flatbuffers::root::<fb_player::Player>(
                black_box(&buf)
            ).unwrap();

            black_box(p);
        })
    });
}

fn config() -> Criterion {
    Criterion::default()
        .sample_size(500)
        .measurement_time(Duration::from_secs(10))
        .warm_up_time(Duration::from_secs(3))
}

criterion_group! {
    name = decode;
    config = config();
    targets =
        decode_pojoc,
        decode_proto,
        decode_capnp,
        decode_fb
}

criterion_main!(decode);