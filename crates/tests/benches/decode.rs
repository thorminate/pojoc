...
fn make_fb_buf(builder: &mut FlatBufferBuilder) -> Vec<u8> {
    let inv_vec = {
        let items = vec![
...
fn make_bebop_buf() -> Vec<u8> {
    let player = player_bebop::Player {
        player_id: Some(1.0),
        level: Some(12.5),
...
fn decode_benchmarks(c: &mut Criterion) {
    ...
    let pojoc_buf = make_pojoc_buf();
    group.bench_function("pojoc", |b| {
        b.iter(|| black_box(pojoc_player::decode(black_box(&pojoc_buf)).unwrap()))
    });

    let proto_buf = make_proto_buf();
    group.bench_function("protobuf", |b| {
        b.iter(|| black_box(proto_player::Player::decode(black_box(proto_buf.as_slice())).unwrap()))
    });

    let capnp_buf = make_capnp_buf();
    group.bench_function("capnp", |b| {
        b.iter(|| {
            let reader = capnp::serialize::read_message(
                black_box(capnp_buf.as_slice()),
                capnp::message::ReaderOptions::new(),
            )
            .unwrap();
            let root = reader.get_root::<capnp_player::player::Reader>().unwrap();
            black_box(root.get_player_id());
        })
    });

    let mut builder = FlatBufferBuilder::new();
    let fb_buf = make_fb_buf(&mut builder);
    group.bench_function("flatbuffers", |b| {
        b.iter(|| black_box(flatbuffers::root::<fb_player::Player>(black_box(&fb_buf)).unwrap()))
    });

    let bebop_buf = make_bebop_buf();
    group.bench_function("bebop", |b| {
        b.iter(|| black_box(player_bebop::Player::deserialize(black_box(&bebop_buf)).unwrap()))
    });
