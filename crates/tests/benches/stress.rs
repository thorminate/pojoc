...
fn make_inventory(n: usize) -> Vec<String> {
...
fn make_proto_buf(n: usize) -> Vec<u8> {
    proto_player::Player {
        player_id: 1.0,
        level: 12.5,
...
fn make_capnp_buf(n: usize) -> Vec<u8> {
...

    {
        ...
        {
            let mut list = p.reborrow().init_inventory(inv.len() as u32);
...
fn make_fb_buf(n: usize, builder: &mut FlatBufferBuilder) -> Vec<u8> {
    let inv = make_inventory(n);
    let inv_vec = {
        let items = inv
            .iter()
...
fn stress_encode(c: &mut Criterion) {
    ...
    for &n in SIZES {
        group.bench_with_input(BenchmarkId::new("pojoc", n), &n, |b, &n| {
            let inv = make_inventory(n);
            let player = Player {
                player_id: 1.0,
                level: 12.5,
        ...
        group.bench_with_input(BenchmarkId::new("protobuf", n), &n, |b, &n| {
            ...
            b.iter(|| black_box(&player).encode_to_vec())
        });

        group.bench_with_input(BenchmarkId::new("capnp", n), &n, |b, &n| {
            b.iter(|| black_box(make_capnp_buf(n)))
        });

        group.bench_with_input(BenchmarkId::new("flatbuffers", n), &n, |b, &n| {
            b.iter(|| {
...
fn stress_decode(c: &mut Criterion) {
    let mut group = c.benchmark_group("stress/decode");

    for &n in SIZES {
        let pojoc_buf = make_pojoc_buf(n);
        let proto_buf = make_proto_buf(n);
        ...
        group.bench_with_input(BenchmarkId::new("protobuf", n), &proto_buf, |b, buf| {
            b.iter(|| black_box(proto_player::Player::decode(black_box(buf.as_slice())).unwrap()))
        });

        group.bench_with_input(BenchmarkId::new("capnp", n), &capnp_buf, |b, buf| {
            b.iter(|| {
                let reader =
                    capnp::serialize::read_message(black_box(buf.as_slice()), ReaderOptions::new())
                        .unwrap();
                black_box(reader)
            })
        });
...
fn stress_roundtrip(c: &mut Criterion) {
    let mut group = c.benchmark_group("stress/roundtrip");
    ...
    for &n in SIZES {
        ...
        let proto_player = proto_player::Player {
            player_id: 1.0,
            level: 12.5,
            status: proto_player::Status::Alive as i32,
            class: proto_player::Class::Warrior as i32,
        ...
        group.bench_with_input(BenchmarkId::new("pojoc", n), &n, |b, _| {
            b.iter(|| {
                let mut buf = Vec::new();
                pojoc_player::encode(&mut buf, black_box(&player));
        ...
        group.bench_with_input(BenchmarkId::new("protobuf", n), &n, |b, _| {
            b.iter(|| {
...
criterion_group! {
    name = stress;
    config = config();
    targets = stress_encode, stress_decode, stress_roundtrip
}
